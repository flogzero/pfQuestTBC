...
SlashCmdList["PFDB"] = function(input, editbox)
  ...
  for i,j in pairs(commandlist) do
    if (i ~= 1) then
      arg2 = arg2 .. j
      if (table.getn(commandlist) ~= (i)) then
        arg2 = arg2 .. " "
      end
    end
  end
  ...
  -- argument: item
  if (arg1 == "item") then
    local maps = pfDatabase:SearchItem(arg2, meta, true)
    pfMap:ShowMapID(pfDatabase:GetBestMap(maps))
  end

  -- argument: vendor
  if (arg1 == "vendor") then
    local maps = pfDatabase:SearchVendor(arg2, meta, true)
    pfMap:ShowMapID(pfDatabase:GetBestMap(maps))
  end

  -- argument: unit
  if (arg1 == "unit") then
    local maps = pfDatabase:SearchMob(arg2, meta, true)
    pfMap:ShowMapID(pfDatabase:GetBestMap(maps))
  end

  -- argument: object
  if (arg1 == "object") then
    local maps = pfDatabase:SearchObject(arg2, meta, true)
    pfMap:ShowMapID(pfDatabase:GetBestMap(maps))
  end

  -- argument: quest
  if (arg1 == "quest") then
    local maps = pfDatabase:SearchQuest(arg2, meta, true)
    pfMap:ShowMapID(pfDatabase:GetBestMap(maps))
  end
